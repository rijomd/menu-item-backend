...
const MiscList = async (req, res) => {
    ...
    let user = req.user;
    try {
        const fields = '_id amountLimit orderLimit countLimit multipleCategorySelection location updatedAt';
        ...
        const lists = MiscList?.length > 0 && [...MiscList].map((Misc) => {
            const { _id, amountLimit, countLimit, orderLimit, multipleCategorySelection, location, updatedAt } = Misc;
            return {
                _id, amountLimit, countLimit, orderLimit, multipleCategorySelection, updatedAt,
